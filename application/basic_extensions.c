...
void basic_TimeEquals(void)
{
    ...
    time_query_mod = get_time_parameter();    
...
Description :  Determine if current time of day is between specified times  template: Time_Between("9:00", "10:00")
Returns     :  Nothing
***************************************************************************/
void basic_TimeBetween(void)
{
    ...
    int iFltIndex;
    char *return_string; 
    int iQueryDeltaMinutes;   
    int iCurrentDeltaMinutes;   
    ...
    // use deltas to handle periods that include the midnight rollover
    iQueryDeltaMinutes = (end_time_query_mod + 1440 - start_time_query_mod)%1440;
    iCurrentDeltaMinutes = (end_time_query_mod + 1440 - time_current_mod)%1440;

    if (iCurrentDeltaMinutes < iQueryDeltaMinutes)  
    {
        // set return variables true
        sprintf(return_string, "true");
        psContext->asIntegerVariables[iIntIndex].iValue = 1;
