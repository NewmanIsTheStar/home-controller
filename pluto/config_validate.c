...
int config_validate(char *filename, CONFIG_CONVERSION_T conversion_table[], int conversion_table_rows, void **configuration_buffer)
{
    ...
    {
        printf("config_validate: error no conversion table row found\n");
        return(-1);
    }
    ...
    err = config_mmap(filename, configuration_buffer, biggest_config_size); 
    ...
    {
        previous_config = config_get_flash_location(filename);
    }

    // upgrade configuration sequentially to latest version 
    for(i=0; i < conversion_table_rows; i++)
    {
        if (latest_valid_config_version < conversion_table[i].version)
        {
